...
function Kpi({ label, value, delta }) {
  return (
    <div
      style={{
        border: "1px solid rgba(234,206,169,0.13)",
        borderRadius: 12,
        background: "rgba(3,3,13,0.35)",
        padding: 14,
      }}
    >
      <div style={{ fontSize: 11, color: "rgba(234,206,169,0.47)", letterSpacing: "0.11em", fontWeight: 600 }}>{label.toUpperCase()}</div>
      <div style={{ fontFamily: "'Syne', sans-serif", color: "#EACEA9", fontSize: 31, marginTop: 4 }}>{value}</div>
      <div style={{ fontSize: 12, color: "#D39758", marginTop: 3 }}>{delta}</div>
    </div>
  );
}
...
function StagePill({ stage }) {
  const tone =
    stage === "Meeting Scheduled"
      ? { border: "rgba(83,227,166,0.5)", bg: "rgba(83,227,166,0.16)", color: "#D39758" }
      : stage === "Intro Requested"
        ? { border: "rgba(211,151,88,0.5)", bg: "rgba(211,151,88,0.16)", color: "#D39758" }
        : stage === "AI Review"
          ? { border: "rgba(242,186,30,0.5)", bg: "rgba(242,186,30,0.16)", color: "#ffd891" }
          : { border: "rgba(234,206,169,0.3)", bg: "rgba(234,206,169,0.08)", color: "rgba(234,206,169,0.8)" };
...
export default function DashboardPage({ onNavigate }) {
  ...
  return (
    ...
    >
      ...
      >
        <div>
          <div style={{ fontSize: 11, color: "#D39758", letterSpacing: "0.18em", fontWeight: 700 }}>DASHBOARD</div>
        ...
        <div style={{ display: "grid", gap: 14, gridTemplateColumns: "minmax(0, 2fr) minmax(0, 1fr)" }}>
          <Card>
            <div style={{ marginBottom: 10 }}>
              ...
              <div style={{ fontSize: 13, color: "rgba(234,206,169,0.52)" }}>Prioritized by fit score and workflow stage.</div>
            </div>
              ...
              <table style={{ width: "100%", borderCollapse: "collapse", minWidth: 630 }}>
                  ...
                  <tr>
                    {[
                      ...
                      <th
                        ...
                        style={{
                          textAlign: "left",
                          borderBottom: "1px solid rgba(234,206,169,0.15)",
                          padding: "10px 10px 9px 0",
                          fontSize: 11,
                          letterSpacing: "0.1em",
                          color: "rgba(234,206,169,0.46)",
                        }}
                      >
                        {title.toUpperCase()}
                      </th>
                ...
                <tbody>
                  {bestPipeline.map((row) => (
                    <tr key={row.id}>
                      <td style={{ borderBottom: "1px solid rgba(234,206,169,0.08)", padding: "12px 10px 12px 0", fontWeight: 600 }}>
                        {row.counterpart}
                      </td>
                      <td style={{ borderBottom: "1px solid rgba(234,206,169,0.08)", padding: "12px 10px 12px 0", color: "rgba(234,206,169,0.84)" }}>
                        {row.type}
                      </td>
                      <td style={{ borderBottom: "1px solid rgba(234,206,169,0.08)", padding: "12px 10px 12px 0" }}>
                        <StagePill stage={row.stage} />
                      </td>
                      <td style={{ borderBottom: "1px solid rgba(234,206,169,0.08)", padding: "12px 10px 12px 0", color: "#D39758", fontWeight: 700 }}>
                        {row.score}%
                      </td>
                      <td style={{ borderBottom: "1px solid rgba(234,206,169,0.08)", padding: "12px 10px 12px 0", color: "rgba(234,206,169,0.84)" }}>
                        {row.owner}
                      </td>
                      <td style={{ borderBottom: "1px solid rgba(234,206,169,0.08)", padding: "12px 0 12px 0", color: "rgba(234,206,169,0.84)" }}>
                        {row.eta}
                      </td>
                    </tr>
                  ))}
          ...
          <div style={{ display: "grid", gap: 14 }}>
            <Card>
              <div style={{ marginBottom: 10 }}>
                ...
                <div style={{ fontSize: 12, color: "rgba(234,206,169,0.52)" }}>News weighted by likely portfolio impact.</div>
              </div>
              <div style={{ display: "grid", gap: 10 }}>
                {NEWS_FEED.map((item) => (
                  <div
                    key={item.id}
                    style={{
                      border: "1px solid rgba(234,206,169,0.12)",
                      borderRadius: 10,
                      padding: "10px 11px",
                      background: "rgba(3,3,13,0.35)",
                    }}
                  >
                    <div style={{ color: "rgba(234,206,169,0.86)", fontSize: 13, lineHeight: 1.4 }}>{item.title}</div>
                    <div style={{ marginTop: 7, display: "flex", justifyContent: "space-between", fontSize: 11 }}>
                      <span style={{ color: "rgba(234,206,169,0.5)" }}>{item.source}</span>
                      <span style={{ color: item.impact === "High" ? "#D39758" : "#D39758" }}>{item.impact} impact</span>
                    </div>
                  </div>
                ))}
              </div>
            ...
            <Card>
              <div style={{ marginBottom: 10 }}>
                ...
                <div style={{ fontSize: 12, color: "rgba(234,206,169,0.52)" }}>Actions from discover/search flows.</div>
              </div>
              <div style={{ display: "grid", gap: 8 }}>
                {activity.length === 0 ? (
                  <div style={{ fontSize: 12, color: "rgba(234,206,169,0.48)" }}>No actions yet. Swipe in Discover to populate this feed.</div>
                ) : (
                  activity.slice(0, 6).map((event) => (
                    ...
                    >
                      <div style={{ color: "rgba(234,206,169,0.9)", fontSize: 13 }}>{event.title}</div>
                      <div style={{ color: "rgba(234,206,169,0.58)", fontSize: 12 }}>{event.detail}</div>
                    </div>
                  ))
                )}
              </div>
