  ...
  return (
    ...
    >
      <div style={{ display: "flex", justifyContent: "space-between", gap: 14, alignItems: "flex-start" }}>
        <div>
          <div style={{ fontFamily: "'Syne', sans-serif", fontSize: 25, color: "#eaceaa" }}>{item.name}</div>
          <div style={{ fontSize: 13, color: "rgba(234,206,170,0.58)", marginTop: 2 }}>{item.summary}</div>
        </div>
      ...
      <div style={{ marginTop: 12, display: "grid", gap: 7 }}>
        {item.reasoning.map((row) => (
          <div key={row} style={{ color: "rgba(234,206,170,0.84)", fontSize: 13 }}>
            • {row}
          </div>
        ))}
      </div>
    ...
    <div style={{ display: "flex", gap: 8 }}>
      {items.map((item) => (
        <button
          ...
          style={{
            borderRadius: 999,
            border: value === item ? "1px solid rgba(211,152,88,0.5)" : "1px solid rgba(234,206,170,0.2)",
            background: value === item ? "rgba(211,152,88,0.2)" : "rgba(234,206,170,0.04)",
            color: value === item ? "#eaceaa" : "rgba(234,206,170,0.7)",
            padding: "7px 11px",
            fontSize: 12,
            cursor: "pointer",
            fontWeight: 700,
...
export default function SearchPage({ onNavigate }) {
  ...
  return (
    <div style={{ minHeight: "100vh", background: "#150c0c", color: "#eaceaa", fontFamily: "'TAN Mon Cheri', serif" }}>
      <link href="https://fonts.googleapis.com/css2?family=Syne:wght@400;500;600;700&display=swap" rel="stylesheet" />

      <header style={{ position: "sticky", top: 0, zIndex: 12, backdropFilter: "blur(10px)", borderBottom: "1px solid rgba(211,152,88,0.25)", background: "rgba(3,3,13,0.93)", padding: "13px 20px", display: "flex", justifyContent: "space-between", alignItems: "center", gap: 12 }}>
        <div>
          <div style={{ fontSize: 11, color: "#d39858", letterSpacing: "0.2em", fontWeight: 700 }}>SEARCH</div>
          <div style={{ fontFamily: "'Syne', sans-serif", fontSize: 27 }}>Personalized semantic search</div>
        </div>
        <button type="button" onClick={() => onNavigate?.("discover")} style={{ border: "1px solid rgba(234,206,170,0.2)", background: "rgba(234,206,170,0.06)", color: "rgba(234,206,170,0.86)", borderRadius: 8, padding: "9px 12px", fontSize: 12, letterSpacing: "0.06em", fontWeight: 700, cursor: "pointer" }}>
          Back to discover
        </button>
      </header>
      ...
      <main style={{ maxWidth: 1080, margin: "0 auto", padding: "20px 20px 30px", display: "grid", gap: 14 }}>
        <div style={{ display: "grid", gap: 10 }}>
          ...
          <div style={{ display: "flex", gap: 10, alignItems: "center" }}>
            ...
            <button type="button" onClick={handleSemanticSearch} style={{ border: "none", background: "linear-gradient(135deg, #85431e, #d39858)", color: "#e8eeff", borderRadius: 8, padding: "8px 12px", fontSize: 12, fontWeight: 700, cursor: "pointer" }}>
              Run AI search
            </button>
          </div>
          {searchLoading && <div style={{ fontSize: 12, color: "rgba(234,206,170,0.54)" }}>Querying backend semantic search...</div>}
          {searchError && <div style={{ fontSize: 12, color: "#f2ba1e" }}>Live search unavailable. Showing local demo results.</div>}
          <div style={{ fontSize: 12, color: "rgba(234,206,170,0.54)" }}>{results.length} results</div>
        </div>
      ...
      {selected && (
        <div role="dialog" style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.6)", display: "grid", placeItems: "center", padding: 20, zIndex: 30 }}>
          <div style={{ width: "min(620px, 100%)", borderRadius: 14, border: "1px solid rgba(211,152,88,0.45)", background: "#150c0c", padding: 18, display: "grid", gap: 12 }}>
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", gap: 8 }}>
              <div>
                <div style={{ fontFamily: "'Syne', sans-serif", fontSize: 28 }}>{selected.name}</div>
                <div style={{ fontSize: 13, color: "rgba(234,206,170,0.6)" }}>{selected.summary}</div>
              </div>
              <button type="button" onClick={() => setSelected(null)} style={{ border: "1px solid rgba(234,206,170,0.2)", background: "transparent", color: "rgba(234,206,170,0.85)", borderRadius: 8, padding: "6px 10px", fontSize: 12, cursor: "pointer" }}>
                Close
              </button>
            </div>

            <div style={{ fontSize: 12, color: "rgba(234,206,170,0.45)", letterSpacing: "0.09em" }}>AI RATIONALE</div>
            <div style={{ display: "grid", gap: 8 }}>
              {selected.reasoning.map((point) => (
                <div key={point} style={{ color: "rgba(234,206,170,0.85)", fontSize: 14 }}>
                  • {point}
                </div>
              ))}
            </div>
            ...
            <div style={{ display: "flex", justifyContent: "flex-end", gap: 8 }}>
              <button type="button" onClick={() => setSelected(null)} style={{ border: "1px solid rgba(234,206,170,0.2)", background: "rgba(234,206,170,0.06)", color: "rgba(234,206,170,0.86)", borderRadius: 8, padding: "8px 11px", fontSize: 12, cursor: "pointer" }}>
                Dismiss
              </button>
              <button type="button" onClick={() => { setSelected(null); onNavigate?.("discover"); }} style={{ border: "none", background: "linear-gradient(135deg, #85431e, #d39858)", color: "#e8eeff", borderRadius: 8, padding: "8px 11px", fontSize: 12, fontWeight: 700, cursor: "pointer" }}>
