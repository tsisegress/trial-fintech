...
function MatchCard({ match, onSkip, onLike, onExplain }) {
  return (
    <article
      style={{
        background: "linear-gradient(180deg, rgba(133,68,30,0.16), rgba(133,68,30,0.04))",
        border: "1px solid rgba(211,151,88,0.34)",
        borderRadius: 16,
        padding: 24,
        minHeight: 380,
    ...
    >
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 16 }}>
        <div>
          <div style={{ fontFamily: "'Syne', sans-serif", fontSize: 30, color: "#EACEA9" }}>{match.name}</div>
          <div style={{ color: "rgba(234,206,169,0.62)", fontSize: 14 }}>{match.oneLiner}</div>
        </div>
        ...
        {match.tags.map((tag) => (
          <span
            ...
            style={{
              fontSize: 11,
              color: "#D39758",
              border: "1px solid rgba(211,151,88,0.35)",
              borderRadius: 6,
              padding: "4px 8px",
...
function ReasonLine({ title, body }) {
  return (
    <div style={{ marginBottom: 10 }}>
      <div style={{ fontSize: 11, color: "rgba(234,206,169,0.46)", letterSpacing: "0.1em", marginBottom: 2 }}>{title.toUpperCase()}</div>
      <div style={{ color: "rgba(234,206,169,0.83)", fontSize: 13, lineHeight: 1.6 }}>{body}</div>
    </div>
  );
}
...
function InfoBlock({ label, value }) {
  return (
    ...
    >
      <div style={{ fontSize: 11, color: "rgba(234,206,169,0.46)", letterSpacing: "0.1em", marginBottom: 4 }}>{label.toUpperCase()}</div>
      <div style={{ color: "rgba(234,206,169,0.9)", fontWeight: 600, fontSize: 13 }}>{value}</div>
    </div>
  );
}
...
function FilterPill({ label, value, options, onChange }) {
  return (
    <label style={{ display: "flex", flexDirection: "column", gap: 6 }}>
      <span style={{ fontSize: 10, letterSpacing: "0.14em", color: "rgba(234,206,169,0.46)", fontWeight: 600 }}>{label.toUpperCase()}</span>
      <select
        value={value}
        ...
        style={{
          ...
          color: "#EACEA9",
          border: "1px solid rgba(234,206,169,0.16)",
          borderRadius: 8,
          padding: "8px 10px",
          fontFamily: "'TAN Mon Cheri', serif",
          fontSize: 13,
    ...
    >
      ...
      <header
        style={{
          ...
          borderBottom: "1px solid rgba(211,151,88,0.25)",
          padding: "14px 28px",
          display: "flex",
      ...
      >
        <div>
          <div style={{ fontSize: 11, color: "#D39758", letterSpacing: "0.18em", fontWeight: 600 }}>DISCOVER</div>
      ...
      <section style={{ maxWidth: 1120, margin: "0 auto", padding: "24px 22px 0" }}>
        <div
          style={{
            ...
            border: "1px solid rgba(234,206,169,0.09)",
            borderRadius: 14,
            padding: 14,
            marginBottom: 16,
          }}
        ...
        <div style={{ color: "rgba(234,206,169,0.5)", marginBottom: 18, fontSize: 13 }}>
        ...
        {matchesLoading && (
          <div style={{ color: "rgba(234,206,169,0.5)", marginBottom: 10, fontSize: 12 }}>
            Syncing latest matches from backend...
          </div>
        )}

        {matchesError && (
          <div style={{ color: "#D39758", marginBottom: 10, fontSize: 12 }}>
